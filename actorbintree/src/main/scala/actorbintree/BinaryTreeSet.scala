  ...
  val normal: Receive = {
    case msg @ Contains(requester, id, target) =>
      if (target == elem)
        ...
        val next = traverse(target)
        subtrees.get(next) match {
          case Some(n) => n ! msg
    ...
    case msg @ Insert(requester, id, target) =>
      if (target == elem) {
        ...
        subtrees.get(next) match {
          case Some(n) => n ! msg
          case None =>
            subtrees += next -> context.actorOf(BinaryTreeNode.props(target, false))
            requester ! OperationFinished(id)
        }
      }
    ...
    case msg @ CopyTo(newRoot) =>
      ...
      subtrees.values.foreach(x => x ! msg)
  }

  def traverse(target: Int): Position = if (target < elem) Left else Right
